...
`config.yaml` は「**コードを変更しなくても、システムの挙動を変えられる値をまとめた場所**」である。

投資戦略は時間とともに見直しが必要になる。例えば「配当利回りの最低ラインを3%から3.5%に上げたい」「スコアリングの重みを財務健全性寄りに変えたい」といった調整のたびにコードを書き直してデプロイし直すのは手間がかかり、ミスのリスクもある。このファイルを変えるだけでLambdaが次回起動時に新しい設定で動くようにすることで、**チューニングを安全・手軽に行える**。

以下の値カテゴリを保持する：

| カテゴリ | 内容 | 変更するタイミング |
|---|---|---|
| **データソース設定** | どのAPIを使うか（J-Quants/EDGAR/FRED）、プラン（light/standard等） | Phase2拡張時、プラン変更時 |
| **スクリーニング閾値** | 「単元購入額上限・配当利回り下限・PER上限」など銘柄を絞り込む条件 | 投資方針の見直し時 |
| **スコアリング重み** | 配当継続性・割安度・財務健全性の評価ウェイト（合計1.0） | 優先順位を変えたい時 |
| **候補生成パラメータ** | 何銘柄を候補とするか、月次予算 | 予算変更時 |
| **既存ポートフォリオ** | 保有銘柄と金額（重複排除に使用）※Phase1は静的 | 購入後の更新時 |
| **AIレポート設定** | 使用するBedrockモデル、最大トークン数、生成の「温度」 | モデル変更・精度調整時 |
| **通知設定** | レポートを送信するメールアドレス | 変更時 |

### 4.2 配置と読み込み

- **格納場所**：S3バケット `investment-config-{env}` 内の `config.yaml`
- **S3を使う理由**：Lambdaのコードに埋め込むより、S3上のファイルを上書きするだけで設定変更が即反映される。Lambdaの再デプロイ（数分かかる作業）が不要
- **バージョニング有効化**：S3のバージョン管理機能を有効にすることで、誤った値に変更してしまった場合でも過去の設定ファイルに戻せる
- **読み込みタイミング**：Lambda関数が起動するたびにS3からダウンロードし、実行中はメモリ上に保持する

**スキーマ検証について**

YAML ファイルは自由に書けてしまうため、誤って `min_dividend_yield: "高め"` のように文字列を書いたり、重みの合計が1.0を超えたりする設定ミスが起きうる。スキーマ検証とは「**設定ファイルの値が正しい型・範囲であるかを、Lambda起動直後に自動チェックする処理**」のことである。

`pydantic` はPythonのライブラリで、「この項目は数値でなければならない」「この値は0より大きく1以下でなければならない」といったルールをコードで定義し、YAMLを読み込んだ時点で自動検証する。

```python
# 検証ルール定義例（イメージ）
class ScreeningConfig(BaseModel):
    max_unit_price_jpy: int          # 整数でなければならない
    min_dividend_yield: float        # 小数でなければならない（例: 0.030）
    min_equity_ratio: float          # 0〜1の間でなければならない
    max_per: float
...
class ScoringWeights(BaseModel):
    ...
    @validator('dividend_continuity', 'valuation', 'financial_health')
    def weights_must_sum_to_one(cls, v, values):
        # 重みの合計が1.0になっているか確認
        ...
```

検証に失敗した場合（値が不正な型、必須項目が欠落、重みの合計が1.0でない等）はLambdaをその場で停止する。これにより「おかしな設定のまま誤った候補銘柄を生成してしまう」事態を防ぐ。Step Functionsがエラーを検知し、SNS経由でメール通知が届く。
...
scoring:
  ...
  # portfolio_correlation は廃止（SP500・高配当日本株も積極選択肢のため）
  # 同一銘柄の重複はスクリーニング層の exclude_existing_tickers で制御
  # 拡張用：資産クラス別の重みを後から追加可能
  asset_class_overrides:
    # reit: {dividend_continuity: 0.45, ...}
...
### 2.1 ○○食品 (2345)
**総合スコア: 0.87**
スコア内訳：配当継続性 0.35 / 割安度 0.30 / 財務健全性 0.22
...
- **用語定義を必ず含める**：PER・PBR・自己資本比率・配当利回りなど、使う指標は毎回定義してから数値を当てはめる
...
- 配当継続性スコア：{score_dividend}（重み40%）
- 割安度スコア：{score_valuation}（重み35%）
- 財務健全性スコア：{score_financial}（重み25%）
...
## 10. データ運用設計（取り込み戦略・削除方針）

### 10.1 J-Quantsデータの取り込み方式

テーブルごとに「全件入れ替え（フルリフレッシュ）」か「追記蓄積（インクリメンタル）」かが異なる。混同すると、削除すべきでないデータを消したり、逆に不要なデータが溜まり続けたりする。

| テーブル | 方式 | 理由 |
|---|---|---|
| `Securities` | **差分Upsert** | 銘柄は追加・上場廃止があるが全量が4,000件程度と少ない。毎週全件を取得し、新規銘柄はInsert、既存は更新、廃止は `delisted_date` をセットする（物理削除はしない） |
| `PriceHistory` | **追記蓄積** | 日次株価は不変の過去データ。毎週「前回取得日の翌日〜今日」の期間だけAPIを叩いて新規レコードを追加。過去データの書き換えはしない |
| `Fundamentals` | **追記蓄積** | 開示情報は不変。`DiscDate` をSKにしているため、同一銘柄の同一日付で重複することはない。新しい四半期開示が出るたびに新規レコードとして追加 |
| `Candidates` | **追記蓄積** | バッチ実行ごとに `run_id` で分離されるため、過去の候補履歴は上書きせず残す |
...
| 削除トリガー | 対象 | 方法 |
|---|---|---|
| 時間経過（自動） | `PriceHistory`（5年超） | **DynamoDB TTL**。`ttl` 属性にUnixtime（取得日+5年）をセット。DynamoDBが非同期で自動削除するため、Lambda側での削除処理不要 |
| 時間経過（自動） | `RunLogs`（180日超） | **DynamoDB TTL**（同上） |
| 時間経過（自動） | `Fundamentals`（7年超） | **DynamoDB TTL**。Light プランの取得可能期間（5年）より長めに保持 |
| 上場廃止 | `Securities` | 物理削除しない。`delisted_date` 属性を追加してソフト削除。過去の `Candidates` が参照しているため |
| J-Quants 解約 | 全テーブルの取得データ | **手動削除**（規約義務）。`aws dynamodb delete-table` で全テーブルを削除する手順を別途ドキュメント化する |
| 開発環境クリーンアップ | dev環境の全データ | `terraform destroy` → DynamoDBテーブルごと削除 |

### 10.4 TTL の動作と注意点

DynamoDBのTTLは「指定した時刻を過ぎたレコードを、DynamoDBが数日以内に非同期で削除する」機能である。削除タイミングは厳密ではない（最大で数日遅れることがある）ため、「TTL経過後でも直前の数日間は古いレコードが残っている可能性がある」ことを念頭に置く。

スコアリング処理では、`disc_date <= バッチ実行日` の絞り込みをクエリ条件に含めることで、TTL未削除の古いレコードが誤って使われることを防ぐ（TTLに頼らず、日付条件で制御する）。

### 10.5 バックアップとリカバリ

| 手段 | 目的 |
|---|---|
| DynamoDB PITR（Point-in-Time Recovery） | 過去35日以内の任意の時点にテーブルを復元可能。誤ってデータを壊した場合のリカバリ |
| S3 バージョニング（config.yaml） | 設定ファイルの誤変更をロールバック |
| RunLogs | バッチ処理の実行記録。「どの範囲のデータが取得済みか」を追跡し、失敗時の再取得範囲を特定 |

**PITRは追加コストが発生する（テーブルサイズに応じて課金）。** 800MB程度の想定規模であれば月数十円以内に収まるが、コスト重視の場合はオフに設定し、S3へのDynamoDB Export（週次）で代替することもできる。
