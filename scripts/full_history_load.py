...
使い方:
  ...
  python scripts/full_history_load.py --skip-prices   # 財務のみ
  python scripts/full_history_load.py --skip-fins     # 株価のみ
"""
import argparse
import gzip
import logging
...
def upload_to_s3(local_path: str, s3_key: str) -> None:
    ...
    log.info(f"  S3保存: s3://{S3_BUCKET}/{s3_key}")
...
def load_securities(client, ddb_client) -> int:
    ...
    log.info(f"  Securities: {written} 件")
    return written


def parse_price_csv(df: pd.DataFrame) -> list[dict]:
    ttl = int(time.time()) + 10 * 365 * 24 * 3600
    records = []
    for _, row in df.iterrows():
...
def load_prices(client, ddb_client, months: list[str], tmpdir: str) -> int:
    log.info(f"PriceHistory Bulk取得: {len(months)} ヶ月分")
    total = 0
    for ym in months:
        path = os.path.join(tmpdir, f"prices_{ym}.csv.gz")
        try:
            client.download_bulk_by_endpoint(endpoint="equities/bars/daily", date=ym, output_path=path)
            upload_to_s3(path, f"{S3_BULK_PREFIX}/prices/{ym}.csv.gz")
            with gzip.open(path, "rt", encoding="utf-8") as f:
                df = pd.read_csv(f)
            records = parse_price_csv(df)
            written = batch_write(ddb_client, f"{TABLE_PREFIX}-PriceHistory", records)
            total += written
            log.info(f"  価格 {ym}: {written} 件")
        except Exception as e:
            log.warning(f"  価格 {ym}: スキップ ({e})")
        finally:
...
def load_fundamentals(client, ddb_client, months: list[str], tmpdir: str) -> int:
    log.info(f"Fundamentals Bulk取得: {len(months)} ヶ月分")
    total = 0
    for ym in months:
        path = os.path.join(tmpdir, f"fins_{ym}.csv.gz")
        try:
            client.download_bulk_by_endpoint(endpoint="fins/summary", date=ym, output_path=path)
            upload_to_s3(path, f"{S3_BULK_PREFIX}/fins/{ym}.csv.gz")
            with gzip.open(path, "rt", encoding="utf-8") as f:
                df = pd.read_csv(f)
            ...
            total += written
            log.info(f"  財務 {ym}: {written} 件（重複除去後/{len(records)}件）")
        except Exception as e:
            log.warning(f"  財務 {ym}: スキップ ({e})")
        finally:
            if os.path.exists(path):
                os.remove(path)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        if not args.skip_securities:
            load_securities(jq, ddb)

        if not args.skip_fins:
            load_fundamentals(jq, ddb, months, tmpdir)

        if not args.skip_prices:
            load_prices(jq, ddb, months, tmpdir)
